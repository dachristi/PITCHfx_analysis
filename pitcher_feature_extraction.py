...
year = 2015
opening_date = datetime.strptime('2015 04 05', '%Y %m %d')
# openning days
# 4/5/2015
# 3/30/2014
# 3/31/2013
...
for file in files:
    ...
    for at_bat in soup.findAll('atbat'):
        ...
        try:
            ...
            pitcher_file = str(pitcher[0])+'.csv'    # toggle pitcher/batter data
            if pitcher_file in listdir('pitcher_data'):    # toggle pitcher/batter data
                at_bat_data.to_csv('pitcher_data/'+pitcher_file, header=False, mode='a', index=False)# toggle pitcher/batter data
            else:
                at_bat_data.to_csv('pitcher_data/'+pitcher_file, header=True, mode='a', index=False)# toggle pitcher/batter data
        except:
            bad_data_frames.append(x)

print len(bad_data_frames)
if len(bad_data_frames)>0:
    print bad_data_frames[:100]
